...
class My_referals extends REST_Controller {
    ...
    public function __construct() {
		...
		$this->table= 'my_referals';
		$this->model_name='my_referals_model';
    ...
    */
    ...
    public function index_post()
    {
		...
		if (!Validator::fails()){
			...
			$data = array(
				...
				'r1' => $input['r1'],
				'r1_earnings' => $input['r1_earnings'],
				'total_earnings' => $input['total_earnings']
			);
			
			$id = $this->Mydb->insert_table_data('my_referals',$data);
			$result = $this->db->get_where('my_referals',array('id'=>$id))->row_array();
	...
	public function register_post(){
		...
		if (!Validator::fails()){
			...
			$emaildata['url_link'] = "https://sartdev.in/pragna/nalaa_organics/interface/account/activation/".$input['token'];
			...
			if($mailsend==1){
					...
					if(isset($input['referal_code'])){
						$token = $input['referal_code'];
					$update = $this->Mydb->update_table_data('customer_references', array('token'=>$token), array("status" => 2));
						// print_R($update);exit();
					}
