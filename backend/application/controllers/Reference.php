    ...
    public function index_post()
    {
		...
		Validator::make($rules);
		 if (!Validator::fails()) {
			...
			$id = $this->Mydb->insert_table_data('reference',$data);
			if($id){
				$data = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
				$value  = withSuccess('Reference Created Successfully',$data);
			}else{
				$value  = withSuccess('Failed to Create Reference',$data);
			}
			$this->response($value, REST_Controller::HTTP_OK);
		}
    } 
...
// print_R($data);exit();
		$is_update = $this->Mydb->update_table_data('reference', array('id'=>$id), $data);
		// print_r($is_update);exit();
		$result = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
		if($is_update>0){
			$value  = withSuccess('Reference Plan Updated Successfully',$result);
		}else{
    ...
    */
    ...
    {
		$result = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
		$res = $this->Mydb->delete_table_data('reference', array('id'=>$id));
		if ($res == 1)
		{	
			$value  = withSuccess('Reference Deleted Successfully',$result);
		}else if ($res == - 1451){
