...
class Customers extends REST_Controller
{
	...
	private function recursive_refs($customerId, $current_level = 1, $max_level = 7, &$result = [])
	{
		...
		foreach ($referrals as $key => $val) {
			...
			$referrals[$key]['level'] = $current_level;
		}
	...
	{
		$message = "success";
		$data = $this->db->order_by('firstName', 'ASC')->get_where("customers")->result_array();
		if (empty($data)) {
	...
	public function register_post()
	{
		...
		if (!Validator::fails()) {
			...
			// }
			$token = '';
			$reference_code = isset($input['referral_code']) ? $input['referral_code'] : '';
			$referredUserId = '';
			if (!empty($reference_code)) {
				// if(strlen($reference_code)==10){
				$q = $this->db->get_where('customers', ['phone' => $reference_code])->row_array();
				if (!empty($q)) {
					$token = randomCodenum(6);
					$referredUserId = $customerId = $q['id'];
					$status = 2;
					$insert = $this->db->insert('customer_references', array('customer_id' => $customerId, 'status' => 2, 'token' => $token, 'createdDate' => cur_date_time()));
					//generate the token and save in customer references
				}
				// }
				// else{
				// 	$q = $this->db->get_where('customer_references', array('token' => $reference_code))->row_array();
	...
	public function index_put($id)
	{
		...
		if (!empty($input['refered_by'])) {
			$rules['refered_by'] = ['Refered By', 'required|numeric'];
			$data['refered_by'] = $input['refered_by'];
		}
...
Nalaa Organic";
						...
						$send_whatsapp = send_whatsapp($customer['phone'], $whatsapp_message, $id, 'user_subscriptions', 'user_subscriptions', $data['planId']);
					}
...
Thank You.";
	...
	function ref_message_get()
	{
		$message = "Hey, I wanted to share this amazing referral for Nalaaorganic for fresh fruits and vegetables at your doorsteps and get amazing discounts on every order.\n\n*Use my mobile number as referral code*\n\nAndroid \n https://play.google.com/store/apps/details?id=io.nalaaorganic.app&hl=en\n\nIOS \nhttps://apps.apple.com/in/app/nalaa-organic/id6499208933\n\nWEB\nhttps://nalaaorganic.com/account/login\n\nPlease reply 'OK' to enable the link.\n\nThank You FROM NALAA";
		$value = withSuccess('Success', ['message' => $message]);
		$this->response($value, REST_Controller::HTTP_OK);
	}
	...
	function checkrefcode_post()
	{
		...
		$token = isset($input['refcode']) ? $input['refcode'] : '';
		if (strlen($token) == 10) {
			$rowData = $this->db->get_where('customers', ['phone' => $token])->row_array();
			if (!empty($rowData['firstName']) && !empty($rowData['email']) && $rowData['status'] == 1) {
				$row = $rowData;
			} else {
				$row = [];
			}
		} else {
			$row = $this->db->get_where('customer_references', array('token' => $token, 'status' => 1))->row_array();
		}
		if (!empty($row)) {
	...
	function checkrefcodeWeb_post()
	{
		$input = $this->input->post();
		;
		$token = isset($input['refcode']) ? $input['refcode'] : '';
		if (strlen($token) == 10) {
			// $row = $this->db->get_where('customers', ['phone'=>$token])->row_array();
			$rowData = $this->db->get_where('customers', ['phone' => $token])->row_array();
			if (!empty($rowData['firstName']) && !empty($rowData['email']) && $rowData['status'] == 1) {
				$row = $rowData;
			} else {
	...
	function update_missing_wallet_amount_post()
	{
		$input = $this->input->post();
		$rules = [
			'subscriptionAmount' => ['Subscription Amount', 'required'],
			'subscriptionPoints' => ['Subscription Points', 'required'],
			'pCfPaymentId' => ['Cashfree Payment ID', 'required'],
			'reference_table' => ['Reference Table', 'required'],
			'reference_id' => ['Reference ID', 'required'],
			'pResponseStatus' => ['Payment Response Status', 'required'],
			'pOrderId' => ['Payment Order ID', 'required'],
			'pEntity' => ['Payment Entity', 'required'],
			'pIsCaptured' => ['Is Payment Captured?', 'required'],
			'pOrderAmount' => ['Order Amount', 'required'],
			'paymentGroup' => ['Payment Group', 'required'],
			'paymentAmount' => ['Paid Amount', 'required'],
			'paymentCurrency' => ['Payment Currency', 'required'],
			'paymentStatus' => ['Payment Status', 'required'],
			'paymentTime' => ['Payment Time', 'required'],
			'paymentMessage' => ['Payment Message', 'required'],
		...
		if (!Validator::fails()) {
			...
			$customerData = $this->db->get_where('customers', ['id' => $input['reference_id']])->row_array();
			if (!empty($customerData)) {
				// get subscription Plan
				$subAmount = $input['subscriptionAmount'];
				$walletPoints = $input['subscriptionPoints'];
				$subscriptionDetails = $this->db->get_where('subscription', ['id'=>$input['subscriptionAmount']])->row_array();
				if(!empty($subscriptionDetails)){
					$subAmount = $subscriptionDetails['basic_amount'];
					$walletPoints = $subscriptionDetails['wallet_points'];
					$addUserSubscription = $this->db->insert('user_subscription', ['user_id'=>$input['reference_id'], 'subscription_id'=>$input['subscriptionAmount'], 'pay_status'=>1, 'status'=>2, 'createdDate'=>cur_date_time()]);
					if($addUserSubscription){

				// Update the comission - BEGINING
				...
				// Update the comission - END
					}
				}
				$customerSubscriptionAmount = [
					'subscriptionAmount' => $customerData['subscriptionAmount'] + $subAmount,
					'subscriptionPoints' => $customerData['subscriptionPoints'] + $walletPoints
				];
				$data = [
					'pCfPaymentId' => $input['pCfPaymentId'],
					'reference_table' => $input['reference_table'],
					'reference_id' => $input['reference_id'],
					'pResponseStatus' => $input['pResponseStatus'],
					'pOrderId' => $input['pOrderId'],
					'pEntity' => $input['pEntity'],
					'pIsCaptured' => $input['pIsCaptured'],
					'pOrderAmount' => $input['pOrderAmount'],
				...
				$insert = $this->Mydb->insert_table_data('payment_details', $data);
				if ($insert) {
					$update = $this->Mydb->update_table_data('customers', ['id' => $input['reference_id']], $customerSubscriptionAmount);
					if ($update) {
						$value = withSuccess('Updated Successfully');
					} else {
						$delete = $this->db->delete('payment_details', ['id' => $insert]);
						$value = withErrors('Unable to Update the subscriptionAmount');
					}
				} else {
					$value = withErrors('Unable to Update Payment Details');
				}
			} else {
				$value = withErrors('No Customer Found');
			}
			$this->response($value, REST_Controller::HTTP_OK);
		}
	}
	...
	// Function to find out if the subscription is there
	function findsubscription_get($id = 0)
	{
		if (!$id) {
			$value = withErrors('Invalid User');
			$this->response($value, REST_Controller::HTTP_OK);
		}
		$userRow = $this->db->get_where('customers', ['id' => $id])->row_array();
		if (empty($userRow)) {
			$value = withErrors('Invalid User, Not Found');
			$this->response($value, REST_Controller::HTTP_OK);
		}
		$subscriptionRows = $this->db->get_where('user_subscription', ['user_id' => $id])->result_array();
		log_message('error', 'User with ID ' . $id . ' has requested for the subscription Notifications');
		//If the subscription is never ever taken
		if (!empty($subscriptionRows)) {
			$subscriptionAmount = $userRow['subscriptionAmount'];
			if ($subscriptionAmount < 500) {
				$msgValue = withSuccess('Success', [
					'data_list' => [
						'value' => "You're low on Subscription Amount, Please Click here to checkout our subscription plans, so that you never miss exciting offers and features like 1.5X subscription points, priority delivery, discounts on each order and many more. Click here to checkout our subscription plans.", 
						'duration' => 3, 
						'url' => 'subscription'
						]
					]);
			} else {
				$msgValue = withErrors('Fail');
			}
		} else {
			$msgValue = withSuccess('Success', [
				'data_list' => [
					'value' => "Hey there!\n
It looks like you haven't subscribed yet, and you're missing out on some fantastic benefits like priority delivery, discounts on each order, 1.5X subscription Points etc. Click here to checkout our subscription plans",
					'duration' => 3,
					'url' => 'subscription'
				]
			]);
		}
		$value = $msgValue ?? withErrors('NAN');
		$this->response($value, REST_Controller::HTTP_OK);
	}
