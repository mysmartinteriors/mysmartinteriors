...
class Sliders extends REST_Controller {
    ...
    */
	...
	function check_add_exist_get(){
		$input = $this->input->get();
		if(!empty($input['slider_name'])){
			$slider_name = $input['slider_name'];
			$sql = "SELECT * from $this->table where $this->table.id >0 && $this->table.slider_name = '$slider_name'";
			// print_R($sql);exit();
    ...
    */
    ...
    public function index_post()
    {
		...
		// print_R($input);exit();
		$rules = [
			'slider_name' => ['Slider Name','required'],
			// 'main_text' => ['Main Text', 'required'],
			// 'shop_url' => ['Shop URL', 'required'],
			'order' => ['Order', 'required'],
			'is_ads' => ['Advertise', 'required'],
			'status' => ['Status','required'],
			'slide_image' => ['Slider Image','required'],
		];
		$message = [
		...
		Validator::make($rules);
		 if (!Validator::fails()) {
			...
			$data = array(	
				'slider_name' =>$input['slider_name'],
				'main_text' =>$input['main_text'],
				'sub_text' =>$input['sub_text'],
				'orders' =>$input['order'],
				'is_ads' =>$input['is_ads'],
				'shop_url' =>$input['shop_url'],
				'status' =>$input['status'],
                'slide_image' => $input['slide_image'],
				'createdDate'=>cur_date_time(),
				// 'created_by'=>$input['created_by'],
			);
		// print_R($data);exit();
			$id = $this->Mydb->insert_table_data('sliders',$data);
			if($id){
				$data = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
				$value  = withSuccess('Slider Created Successfully',$data);
			}else{
				$value  = withSuccess('Failed to Create Slider',$data);
			}
			$this->response($value, REST_Controller::HTTP_OK);
		}
    } 
    ...
    */
    ...
    public function index_put($id)
    {
		...
		if(!empty($input['slider_name'])){
			$rules['slider_name'] = ['Slider Name','required'];
			$data['slider_name'] = $input['slider_name'];
		}
		if(!empty($input['main_text'])){
			$rules['main_text'] = ['Main Text','required'];
			$data['main_text'] = $input['main_text'];
		}
		if(!empty($input['sub_text'])){
			$rules['sub_text'] = ['Sub Text','required'];
			$data['sub_text'] = $input['sub_text'];
		}
		if(!empty($input['shop_url'])){
			$rules['shop_url'] = ['Shop Url','required'];
			$data['shop_url'] = $input['shop_url'];
		}
		if(!empty($input['slide_image'])){
			$rules['slide_image'] = ['Slider Image','required'];
			$data['slide_image'] = $input['slide_image'];
		}
		if(!empty($input['order'])){
			$rules['order'] = ['Order','required'];
			$data['orders'] = $input['order'];
		}
		if(!empty($input['is_ads'])){
			$rules['is_ads'] = ['Advertiise','required'];
			$data['is_ads'] = $input['is_ads'];
		}
...
// print_R($data);exit();
		$is_update = $this->Mydb->update_table_data('sliders', array('id'=>$id), $data);
		// print_r($is_update);exit();
		$result = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
		if($is_update>0){
			$value  = withSuccess('Sliders updated successfully',$result);
		}else{
    ...
    */
    ...
    public function index_delete($id=0)
    {
		if(!empty($id)){
			$result = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
			$res = $this->Mydb->delete_table_data('sliders', array('id'=>$id));
			if ($res == 1)
			{	
				$value  = withSuccess('Slider Deleted Successfully',$result);
			}else if ($res == - 1451){
			...
			$value = withErrors('Slider cannot delete');
		}
		$this->response($value, REST_Controller::HTTP_OK);
    }
