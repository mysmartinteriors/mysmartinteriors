...
class Testimonial extends REST_Controller {
    ...
    public function __construct() {
        ...
        $this->model_name='testimonial_model';   
    ...
    */
    ...
    public function index_post()
    {
		...
		// print_R($data);exit();
			$id = $this->Mydb->insert_table_data('testimonials',$data);
			if($id){
				$data = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
				$value  = withSuccess('Testimonial Created Successfully',$data);
			}else{
				$value  = withSuccess('Failed to Create Testimonial',$data);
			}
			$this->response($value, REST_Controller::HTTP_OK);
		}
    } 
    ...
    */
    ...
    public function index_put($id)
    {
		...
		if(!empty($input['description'])){
			$rules['description'] = ['Description','required'];
...
// print_R($data);exit();
		$is_update = $this->Mydb->update_table_data('testimonials', array('id'=>$id), $data);
		// print_r($is_update);exit();
		$result = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
		if($is_update>0){
			$value  = withSuccess('Testimonial updated successfully',$result);
		}else{
    ...
    */
    ...
    public function index_delete($id=0)
    {
		if(!empty($id)){
			$result = $this->Mydb->get_single_result($id,$this->table,$this->model_name);
			$res = $this->Mydb->delete_table_data('testimonials', array('id'=>$id));
			if ($res == 1)
			{	
				$value  = withSuccess('Testimonial Deleted Successfully',$result);
			}else if ($res == - 1451){
			...
			$value = withErrors('Testimonial cannot delete');
		}
		$this->response($value, REST_Controller::HTTP_OK);
    }
