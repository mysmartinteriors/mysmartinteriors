...
class Products_model extends CI_Model
{
	function __construct() { 
		...
		$this->ratings_table = "product_ratings"; 
    } 
	...
	function filter($filter_data, $item=1, $page=1,$sortby='',$orderby='',$all=true) {
		 ...
		 if(!empty($filter_data)){
			foreach ($filter_data as $k => $v) {
				...
				if(($v['type']=='scat_type')&&($v['value']!="")){
					...
					$value = $v['value'];
					if ($ct > 1) {
						$q .= " OR cc.code like '%" . $value . "%'";
					} else {
						$q .= " ) AND (cc.code like '%" . $value . "%'";
					}
					unset($filter_data[$k]);
				}
				if(($v['type']=='min_price')&&($v['value']!="")){
							$value = $v['value'];
						$q .= " ) AND ($this->table.price>=".$value;
						unset($filter_data[$k]);
				}
				if(($v['type']=='max_price')&&($v['value']!="")){
							$value = $v['value'];
						$q .= " ) AND ($this->table.price<=".$value;
						unset($filter_data[$k]);
				}
				if (($v['type'] == 'sort')&&($v['value']!="")) {
					$value = $v['value'];
					if ($value == "lowtohigh") {
						$orderby = " AND $this->table.price ASC ";
					}else if ($value == "hightolow") {
						$orderby = " AND $this->table.price DESC ";
					}else if ($value == "newest") {
						$orderby = " ORDER BY $this->table.updatedDate DESC ";
					}else {
						$orderby = " ORDER BY $this->table.updatedDate DESC ";
					}
					unset($filter_data[$k]);
        ...
        if (($item == 0) && ($page == 0)) {
            $sql = "select count($this->table.id) as countt 
					...
					INNER JOIN $this->category_table cc ON cc.id=$this->table.categoryId
					INNER JOIN $this->category_table cp ON cp.id=$this->table.parentId	
                    Where($this->table.id>0";
        } else {
            $sql = "select $this->table.*, cc.text as categoryName,
					cp.text as parentName,cc.code as s_code, cp.code as p_code,
					...
					left join $this->users_table as created_users_table on  $this->table.created_by = created_users_table.id
					left join $this->users_table as updated_users_table on   $this->table.updated_by = updated_users_table.id
					LEFT JOIN $this->ratings_table ON $this->table.id=$this->ratings_table.productId
					where ($this->table.id>0";
				}
		if(!empty($filter_data)){
			foreach ($filter_data as $k => $v) {
