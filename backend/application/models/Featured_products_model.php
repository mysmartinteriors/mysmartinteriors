...
class Featured_products_model extends CI_Model
{
	...
	function filter($filter_data, $item=1, $page=1,$sortby='',$orderby='',$all=true) {
         $search_field = array("$this->product_table.name", "$this->product_table.code");
        if (($item == 0) && ($page == 0)) {
            $sql = "select count($this->table.id) as countt 
					...
					left join $this->product_table ON $this->table.productId = $this->product_table.id
					INNER JOIN $this->category_table cc ON cc.id=$this->table.categoryId
                    Where($this->table.id>0";
        } else {
			// $sql = "SELECT fp.featureId,fp.status as fp_status,p.*,c.text as categoryName

            $sql = "select $this->table.status as fp_status, $this->table.id as fpId, cc.text as categoryName,cc.code as s_code, $this->product_table.*
					from $this->table 
					LEFT JOIN $this->product_table ON $this->table.productId = $this->product_table.id
					INNER JOIN $this->category_table cc ON cc.id=$this->table.categoryId
					left join $this->lookups_table as status_table on $this->table.status = status_table.id 
