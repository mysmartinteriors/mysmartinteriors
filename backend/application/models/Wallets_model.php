	...
	function filter($filter_data, $item=1, $page=1,$sortby='',$orderby='',$all=true) {
        ...
        if (($item == 0) && ($page == 0)) {
            $sql = "select count($this->table.id) as countt 
					FROM $this->table 
					left join $this->plan_table as plan_table on $this->table.planId = plan_table.id
            ...
            $sql = "select $this->table.*, plan_table.name as plan_name, plan_table.wallet_points as wallet_amount, customer_table.firstName as customerName, status_table.l_value as status_name, status_table.color_name as status_color_name
					from $this->table 
					left join $this->plan_table as plan_table on $this->table.planId = plan_table.id
					left join $this->customer_table as customer_table on $this->table.customerId = customer_table.id
					left join $this->lookups_table as status_table on $this->table.status = status_table.id
