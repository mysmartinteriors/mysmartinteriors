 ...
 * @version         3.0.0
 */
abstract class Custom_REST_Controller extends CI_Controller {
     ...
     */
    public function __construct($config = 'custom_rest')
    {
        parent::__construct();
    ...
    protected function _parse_post()
    {
        ...
        if ($this->input->server('CONTENT_TYPE') === 'application/json') {
            ...
            $this->_post_args = json_decode($raw_input_stream, true);

            // Check for JSON decoding errors
            if ($this->_post_args === null) {
                $error_message = json_last_error_msg();
                $this->response(['status' => 'error', 'message' => 'Invalid JSON data. Error: ' . $error_message], 400);
            }
            // Set $_POST to match the decoded JSON data
            $_POST = $this->_post_args;
        } else {
            // Default behavior for non-JSON content type
            $this->_post_args = $_POST;
        }
        // if ($this->request->format) {
        //     $this->request->body = $this->input->raw_input_stream;
