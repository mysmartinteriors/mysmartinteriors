...
class categorymodel extends CI_Model{
    ...
    function savecategory($data){
        ...
        if($id!=''){
            ...
            $logData['description']='updated the category '.$data['text'];
        }else{
            ...
            $logData['description']='created a new category '.$data['text'];
        }
        if($res>0 && $id!=''){
            $code=create_slug($data['text']);
            $up_data['code']=$code.'.'.$id;
            $where = array('id' => $id);
            $this->adminmodel->update_table_data('categories',$where,$up_data);
