...
class Repurchase extends CI_Controller {
	...
	function __construct(){
		...
		$this->admin->nocache(); 
        $this->module_name='repurchase';
		$this->view_path = "admin/repurchase";
	}
	
    public function index() {
        $data = $this->admin->commonadminFiles();
		$data['title']="repurchase";
		$this->load->view("admin/repurchase/repurchase_view",$data);
    }
...
//  print_R($filter_data);exit();
        ...
        $str=$this->load->view("admin/repurchase/repurchase_tbl_view",$data,true);
    ...
    function add_new(){
        ...
        if($id!=''){
            $where = array('id' => $id );
            $apidata = $this->curl->execute("repurchase/$id", "GET");
            if($apidata['status'] == 'success' && !empty($apidata['data_list'])){
                $data['dataQ'] = $apidata['data_list'];
            }
            // $data['dataQ']=$this->adminmodel->get_table_data('repurchase',$where,'*',true);
        }
        $str=$this->load->view("admin/repurchase/add_repurchase_view",$data,true);
        $value=array(
            'status'=>'success',
            'message'=>$str
        );
    ...
    function save_add_data(){
        ...
        if(!empty($id)){
            $apidata = $this->curl->execute("repurchase/$id", "PUT", $data);
        }else{
            $apidata = $this->curl->execute("repurchase", "POST", $data);
        }
        if($apidata['status'] == 'success' && !empty($apidata['data_list'])){
     ...
     function delete_repurchase(){
        // check_permission(4,'delete','json');
        $id=$this->input->post("id");
        $status='';
        $msg='Cannot delete at this time...';
        if($id!=""){ 
            $apidata = $this->curl->execute("repurchase/$id", "DELETE");
            if($apidata['status'] == 'success' && !empty($apidata['data_list'])){
                $status = $apidata['status'];
                $message = $apidata['message'];
            }else{
