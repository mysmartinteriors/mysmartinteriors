$(document).ready(function(){	    
	...
	$('.new-folder').click(function(){
		...
		$.ajax({
			  ...
			  data: {csrf_token : csrf_token, path: folder_path, path_thumb: folder_path_thumb}
			}).done(function( msg ) {
			window.location.href = $('#refresh').attr('href') + '&' + new Date().getTime();
		});
		}
...
function delete_file(file1,file2) {
    $.ajax({
	    type: "POST",
	    url: "delete_file.php",
	    data: {csrf_token : csrf_token, path: file1, path_thumb: file2}
	}).done(function( msg ) {
    });
}
...
function delete_folder(folder1,folder2) {
    $.ajax({
	    ...
	    data: {csrf_token : csrf_token, path: folder1, path_thumb: folder2}
	}).done(function( msg ) {
    });
}
