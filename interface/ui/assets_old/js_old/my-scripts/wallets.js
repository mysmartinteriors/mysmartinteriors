...
function add_wallets() {
  $(".btn-addData").on("click", function (e) {
    ...
    if (id != '' && id > 0) {
      var modal_title = 'Edit wallets Plan';
    } else {
      var modal_title = 'Add New wallets Plan';
    }
    ...
    mediaDialog.init(function () {
      setTimeout(function () {
        $.post(urljs + "admin/wallets/add_new", { 'id': id }, function (data) {
          if (data.result > 0) {
...
function save_add_data() {
  $('#addedit_slider_form').validate({
    ...
    submitHandler: function (e) {
      ...
      $["ajax"]({
        url: urljs + "admin/wallets/save_add_data",
        type: "POST",
        dataType: "json",
        data: formdata,
        contentType: false,
...
function delete_data() {
  $('.delData').unbind().click(function () {
    ...
    swal({
      title: "Are you sure?",
      text: "You will not be able to recover this wallets plan!",
      type: "warning",
      ...
      if (isConfirm) {
        ...
        $.post(urljs + 'admin/wallets/delete_wallets', { 'id': mappingId }, function (data) {
          closeajax();
          if (data.status == 'success') {
            get_datas();
            show_toast('success', data.message, data.message)
          } else {
