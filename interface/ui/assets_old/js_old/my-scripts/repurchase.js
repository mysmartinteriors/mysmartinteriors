...
function add_repurchase() {
  $(".btn-addData").on("click", function (e) {
    e.preventDefault();
    var id = $(this).attr('data-id');
    if (id != '' && id > 0) {
      var modal_title = 'Edit repurchase Plan';
    } else {
      var modal_title = 'Add New repurchase Plan';
    }
    ...
    mediaDialog.init(function () {
      setTimeout(function () {
        $.post(urljs + "admin/repurchase/add_new", { 'id': id }, function (data) {
          if (data.result > 0) {
...
function delete_data() {
  $('.delData').unbind().click(function () {
    ...
    swal({
      title: "Are you sure?",
      text: "You will not be able to recover this repurchase plan!",
      type: "warning",
      ...
      if (isConfirm) {
        ...
        $.post(urljs + 'admin/repurchase/delete_repurchase', { 'id': mappingId }, function (data) {
          closeajax();
          if (data.status == 'success') {
            get_datas();
