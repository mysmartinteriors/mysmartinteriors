...
function get_datas(page) {
  var url = "admin/subscription/get_datas";
  ajax_filter(url, page, render_data, false)
}

function render_data(datas) {
  $("#subscriptionTbl").html(datas.str);
  filters();
  delete_data();
  add_subscription();
}
...
function add_subscription() {
  $(".btn-addData").on("click", function (e) {
    ...
    if (id != '' && id > 0) {
      var modal_title = 'Edit Subscription Plan';
    } else {
      var modal_title = 'Add New Subscription Plan';
    }
    ...
    mediaDialog.init(function () {
      setTimeout(function () {
        $.post(urljs + "admin/subscription/add_new", { 'id': id }, function (data) {
...
function save_add_data() {
  $('#addedit_slider_form').validate({
    ...
    submitHandler: function (e) {
      ...
      $["ajax"]({
        url: urljs + "admin/subscription/save_add_data",
        type: "POST",
        dataType: "json",
        data: formdata,
        contentType: false,
  ...
  $('.delData').unbind().click(function () {
    ...
    swal({
      ...
      if (isConfirm) {
        ...
        $.post(urljs + 'admin/subscription/delete_subscription', { 'id': mappingId }, function (data) {
          closeajax();
          if (data.status == 'success') {
            get_datas();
